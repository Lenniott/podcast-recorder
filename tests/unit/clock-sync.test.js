...
describe('createClockSync', () => {
  ...
  it('offset becomes the mean of the 3 round-trip samples once all 3 pongs land', () => {
    ...
    clock.handlePong({ seq: sent[1].seq, serverReceivedAt: 1200 })
    clock.handlePong({ seq: sent[2].seq, serverReceivedAt: 1300 })
    expect(clock.offset).toBeCloseTo(200, 5)
  })
  ...
  it('a stray or duplicate seq is ignored and does not affect the offset', () => {
    ...
    clock.handlePong({ seq: 999, serverReceivedAt: 5000 })
    clock.handlePong({ seq: sent[0].seq, serverReceivedAt: 1100 })
    clock.handlePong({ seq: sent[0].seq, serverReceivedAt: 9999 })
