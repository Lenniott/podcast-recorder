...
test('collapsing hides the mic panel but keeps record and clap', async ({ page }) => {
  ...
  await expect(page.getByRole('combobox', { name: 'Mic' })).toBeVisible()
  await expect(page.getByRole('button', { name: 'Start Recording' })).toBeVisible()
  await expect(page.getByRole('button', { name: 'Sync Tone Marker' })).toBeVisible()

  await page.getByRole('button', { name: 'Collapse sidebar' }).click()

  await expect(page.getByRole('combobox', { name: 'Mic' })).toHaveCount(0)
