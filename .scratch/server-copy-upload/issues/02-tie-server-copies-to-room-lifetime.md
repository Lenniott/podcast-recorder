...
- [ ] Room expiry is based on the existing room-created-at lifetime configured by the environment.
- [ ] Upload and download requests for expired or deleted rooms are rejected.
- [ ] Expired-room cleanup removes room metadata and any server-copy files for that room.
- [ ] Manual room deletion also removes any server-copy files for that room.
- [ ] Production configuration exposes the room lifetime setting used by the app.
- [ ] Tests cover expiry/deletion cleanup without requiring a real recording.
