...
- [ ] Stopping a recording with an incomplete server copy opens a modal showing percentage remaining.
- [ ] The modal clearly says the local recording is saved locally.
- [ ] Closing, refreshing, changing URL, or navigating in-app while upload is incomplete triggers a warning.
- [ ] The upload warning is less severe than the active-recording warning and says leaving means sending the local file another way.
- [ ] The modal closes automatically when the server copy is complete.
- [ ] Leaving without active recording or incomplete upload remains unblocked.
