...
- [ ] Upload starts only after local recording has started and a server-copy session has been accepted for the active room.
- [ ] Audio chunks are queued for upload only after the local writer confirms they were written.
- [ ] Server acknowledgements advance upload progress by byte or sample offset, not by wall-clock time.
- [ ] Upload failure or slowness never blocks, delays, corrupts, or fabricates local recording audio.
- [ ] The client can report server-copy percentage as acknowledged server audio divided by confirmed local audio.
- [ ] Tests prove local recording still proceeds when server-copy upload is slow or rejected.
